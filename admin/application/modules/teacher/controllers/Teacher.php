...
// onlinelesson
   ...
   // pdf word file download also dynamic teacher name changes
    public function download($id){
        
        $fileinfo = $this->Teacher_model->download($id);
        $teachername = $fileinfo[0]->name;
        //$teachername = $fileinfo['name'];

        $file = 'uploads/resume/'.$fileinfo[0]->file_src;
        ...
        header('Content-Disposition: attachment; filename="'.$teachername.'.'.$ext.'"');
        readfile($file);
 ...
 //        $tofile= realpath("uploads/resume/".$id);
...
// pdf word file only download but name not change
       // public function download($id){
    //     $this->load->helper('download');
    //     $alldata = $this->Teacher_model->get_data();
    //     $teachername = $alldata[0]->name;

    //     $fileinfo = $this->Teacher_model->download($id);
    //     $file = 'uploads/resume/'.$fileinfo['file_src'];
