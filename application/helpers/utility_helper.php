...
if (! function_exists('student_is_logged_in')) {
    function student_is_logged_in()
    {
        $CI =& get_instance();  // || $student_is_logged_in == false
        $student_is_logged_in = $CI->session->userdata('student_is_logged_in');

        if (!isset($student_is_logged_in) || $student_is_logged_in == 0) {
            //redirect('home');
           redirect('login');
        }
    }
...
############################# if not login Parent  ####################################
if (! function_exists('parent_is_logged_in')) {
    function parent_is_logged_in()
    {
        $CI =& get_instance();  // || $student_is_logged_in == false
        $parent_is_logged_in = $CI->session->userdata('parent_is_logged_in');

        if (!isset($parent_is_logged_in) || $parent_is_logged_in == 0) {
            //redirect('home');
           redirect('login');
        }
    }
...
############################# if not login teacher  ####################################
if (! function_exists('teacher_is_logged_in')) {
    function teacher_is_logged_in()
    {
        $CI =& get_instance();  // || $student_is_logged_in == false
        $teacher_is_logged_in = $CI->session->userdata('teacher_is_logged_in');

        if (!isset($teacher_is_logged_in) || $teacher_is_logged_in == 0) {
            //redirect('home');
           redirect('login');
        }
    }
...
############################# testimonial data ##############################
if (! function_exists('get_testimonial_data')) {
...
############################# popular subject data ##############################

if (! function_exists('get_popularsubjects')) {
    function get_popularsubjects()
    {
        $CI =& get_instance();
        //$CI->db->select('');
        $qry="SELECT * FROM tbl_subjects where popular_subjects='1'";
        $query = $CI->db->query($qry);
        $data_row= $query->result();
        return $data_row;
    }
...
if (! function_exists('get_allsubjects')) {
    function get_allsubjects()
    {
        $CI =& get_instance();
        //$CI->db->select('');
        $qry="SELECT * FROM tbl_subjects where popular_subjects='0'";
        $query = $CI->db->query($qry);
        $data_row= $query->result();
        return $data_row;
    }
...
###########  Home findtutor data searching (subject wise teacher display ) ##################
if (! function_exists('get_teacher')) {
    function get_teacher($id) {

        $CI =& get_instance();
        $qry="SELECT tbl_signup.*,
                tbl_signup.id AS sign_id,
                tbl_signup.name AS teacher_name,
                tbl_subjects.id AS subject_id,
                tbl_subjects.name AS subject_name,
                tbl_levels.name AS levels_name,
                tbl_grade.name AS grade_name
                FROM tbl_subjects
                INNER JOIN tbl_qualifications ON tbl_subjects.id=tbl_qualifications.subject_id 
                INNER JOIN tbl_signup ON tbl_qualifications.teacher_id = tbl_signup.id
                INNER JOIN tbl_levels ON tbl_qualifications.levels_id = tbl_levels.id
                INNER JOIN tbl_grade ON tbl_qualifications.grade_id = tbl_grade.id
                WHERE tbl_subjects.id='".$id."'";
        $query = $CI->db->query($qry);  //'".$id."'
        $data_row= $query->result();
        return $data_row;
...
// if (! function_exists('get_googleloginstudent')) {
//     function get_googleloginstudent()
//     {
//         $CI =& get_instance();
//         //$CI->db->select('');
//         // $qry="SELECT * FROM google_login where usertype_id='1' ORDER BY 'id' DESC LIMIT 1 //id=$id";
//         $qry="SELECT * FROM google_login WHERE usertype_id='1'";
//         $query = $CI->db->query($qry);
//         $data_row= $query->result();
//         return $data_row;
//     }
// }
    ...
    ###################### google login(from tbl signup) student ##############################

// if (! function_exists('get_googleloginsignupidstudent')) {
//     function get_googleloginsignupidstudent($googleid)
//     {
//         $CI =& get_instance();
//         //$CI->db->select('');
//         $qry="SELECT google_login.*,
//         tbl_signup.*,
//         tbl_signup.id as sign_id
//         FROM tbl_signup
//         INNER JOIN google_login ON tbl_signup.googlelogin_id=google_login.id
//         WHERE googlelogin_id=$googleid 
//         ";
//         //tbl_signup.googlelogin_id=$googleid AND tbl_signup.usertype_id='1'
//         $query = $CI->db->query($qry);
//         $data_row= $query->result();
//         return $data_row;
//     }
// }
 ...
 if (! function_exists('get_faq')) {
    function get_faq($id)
    {
        $CI =& get_instance();
        //$CI->db->select('');
        $qry="SELECT * FROM tbl_faq INNER JOIN tbl_faq_topics ON tbl_faq.faq_topic_id=tbl_faq_topics.id WHERE faq_topic_id=$id AND tbl_faq.status='1'";
        $query = $CI->db->query($qry);
        $data_row= $query->result();
        return $data_row;
    }
...
#########################  related blog ############################
 if (! function_exists('get_relatedblog')) {
    function get_relatedblog($id)
    {
        $CI =& get_instance();
        //$CI->db->select('');
        $qry="SELECT * FROM tbl_blog INNER JOIN tbl_blog_category ON tbl_blog.blog_cat_id=tbl_blog_category.id WHERE tbl_blog.id=$id AND tbl_blog.status ='1'";
        $query = $CI->db->query($qry);
        $data_row= $query->result();
        return $data_row;
    }
