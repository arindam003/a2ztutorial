        ...
        if($this->input->server('REQUEST_METHOD') == 'POST'){
                ...
                if($this->form_validation->run()== TRUE){
                        ...
                        if (! $this->upload->do_upload()) {
                         $error = array('error' => $this->upload->display_errors()); 
                            ...
                            // $fname = $_FILES['userfile']['file_name'];
                            //   echo '<pre>';
                             // print_r($config);die;
                              //echo $fname;// die;
                             // print_r($final_files_data);//die;
                              //print_r($final_files_data['file_ext']); //die;
                             ...
                             // print_r($_FILES);die;
                              ...
                              //$data['file_src']= $final_files_data[0]['fname'];

                              

                              // $data['file_src']= explode(" ", $file_src);
                              //print_r($data['file_src']).'pdf';
                             // die;

                            // $file_name=$_FILES['userfile']['file_name'];
                            // $ff = $final_files_data['file_ext'];
                            
                            //$data['file_src']= $file_name.$ff;
                             //$data['file_src']=$final_files_data[0].$data['name'];
                             //$data['file_src']=$data['name'].$config['file_ext'];
                        }
           ...
           // print_r($data);die;
            ...
            if($data_inserted){
                ...
                $to = "jayanti.mahetechnologies@gmail.com";
                ...
                $message .= "Regards,<br>".$data['name'].',<br>'.
                             // '<a href=" '.base_url('tutors/login_confirmation').'">Confirmation </a>'

                            // '<a href=" '.base_url('tutors/login_confirmation')."/".$data['name']."/".$code.'">Confirmation Account</a>'
                        '<a href=" '.base_url('tutors/loginconfirmation')."/".
                            $data['name'].'">Confirmation Account</a>'
...
public function loginconfirmation22() { 
    $uname =  $this->uri->segment(3);
        //$data['alldata'] = $this->Tutors_model->get_data();
        ...
        {
               $data['mail_status'] = '0';
               $data['signup_data'] = $this->Tutors_model->update_maildata($data,$uname);
               // echo '<pre>';
               // print_r($data); die;
               $this->session->set_flashdata('success_msg', 'Data Updated Successfully'); 
             
               $this->template->set('title','Login Confirmation ');
              $this->template->set_layout('layout_main','front');  
              $this->template->build('loginconfirm',$data);          
        }
    }
    ...
    public function loginconfirmation(){
        ...
        //  print_r($uname) ;die;
         ...
         // if ($this->input->server('REQUEST_METHOD') == 'POST'){
         //    $data['mail_status']= strip_tags($this->input->post('mail_status'));
         // }
         // $data_inserted = $this->Tutors_model->edit_profilemail($data,$uname);
         //redirect('loginconfirmation/loginconfirm');

        //  $this->form_validation->set_rules('mail_status', 'mail_status', 'trim|required');
        //  if($this->form_validation->run() == TRUE){ 
        //      $data_inserted = $this->Tutors_model->edit_profilemail($data,$uname);
        //      $this->session->set_flashdata('success_msg', ' Edited Successfully'); 
        //     redirect('loginconfirmation/loginconfirm');
        // }
                $data['signup_data'] = $this->Tutors_model->get_signup($uname);
               //print_r($data['signup_data']) ;die;
                $this->template->set('title','Login Confirmation ');
                $this->template->set_layout('layout_main','front');
                $this->template->build('loginconfirm',$data);
    }
    ...
    public function mail_inactive($id)
    {
        if($this->uri->segment(3)=="")
        {
            $this->session->set_flashdata('err_msg', 'Dont Change the URL physically'); 
        ...
        {
               $data['mail_status'] = '0';  // inactive
               $data_inserted = $this->Tutors_model->edit_profilemail($data,$id);
               $this->session->set_flashdata('success_msg', ' edited Successfully'); 
              redirect($_SERVER['HTTP_REFERER']);             
        }
    }

public function mail_active($id)
    {
        if($this->uri->segment(3)=="")
        {
            $this->session->set_flashdata('err_msg', 'Dont Change the URL physically'); 
...
public function activate(){
        ...
        $user = $this->Tutors_model->get_signup($uname);
 // echo '<pre>';
 // print_r($user); //die; 

       // print_r($user['mail_verify']);die;
        //if code matches 526707
        // if($user['code'] == $code){
        if($user['mail_verify'] == $code){
            //echo 'hi';die; 
            //update user active status
            $data['mail_status'] = 0;
            $data['mailstatus'] = $this->Tutors_model->update_mailstatus($data, $uname);
            ...
            $this->session->set_flashdata('message', 'Cannot activate account. Code didnt match');
             //redirect(base_url('tutors/activate'));
        }
       
        $this->template->set('title','Login Confirmation ');
        $this->template->set_layout('layout_main','front');
        $this->template->build('loginconfirmation',$data);
