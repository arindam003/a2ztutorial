<?php
 ...
 function get_googleprofile(){
	...
	//function get_signup_profile($googlemail){
		...
		$this->db->join('google_login','tbl_signup.id=google_login.signup_id','INNER');
		$this->db->where('tbl_signup.usertype_id','1');
		$this->db->where('google_login.email',$googlemail);
		$this->db->where('google_login.id',$googleid);
		//$this->db->where('tbl_signup.signup_id',$googlesignupid);
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		$result = $query->result();
		return $result;
